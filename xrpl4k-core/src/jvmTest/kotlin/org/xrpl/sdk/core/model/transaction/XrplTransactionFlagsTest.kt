...
class XrplTransactionFlagsTest : FunSpec({
    ...
    context("Unsigned with flags") {
        ...
        test("flags participates in hashCode") {
            val a =
                XrplTransaction.Unsigned(
                    transactionType = TransactionType.Payment,
                    account = account,
        ...
        test("toString includes flags") {
            ...
            tx.toString().contains("flags=") shouldBe true
        }
    }

    // ── Filled.create with flags ─────────────────────────────────────────────

    context("Filled.create with flags") {
        ...
        test("Filled hashCode includes flags") {
            val a =
                XrplTransaction.Filled.create(
                    transactionType = TransactionType.Payment,
                    account = account,
        ...
        test("Filled toString includes flags") {
            val filled =
                XrplTransaction.Filled.create(
                    transactionType = TransactionType.Payment,
                    account = account,
            ...
            filled.toString().contains("flags=99") shouldBe true
        }
    }
})
